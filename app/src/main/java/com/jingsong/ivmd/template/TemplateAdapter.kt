...
class TemplateAdapter<A>(private val context: Context) : BaseAdapter<A>() {
    ...
    override fun onBindViewHolder(p0: BaseViewHolder, p1: Int) {
        ...
        val entity = dataList?.get(p1) as TemplateRowModel

        tvId.text = "${p1 + 1}"
        tvIp.text = if (TextUtils.isEmpty(entity.peopleName)) "" else entity.peopleName
        tvTime.text = TimeUtil.getDayByType(entity.createTime, TimeUtil.DATE_YMD_HMS)

        Glide.with(context).load(entity.imageUrl).asBitmap().override(120, 100)
            .into(ivImg)
    }
}